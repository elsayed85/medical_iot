...
<body>
  ...
  <form method="POST" action="{{ route('login') }}">
	...
	<div class="inputGroup inputGroup1">
		...
		<input type="text" id="email" name="email" class="email" value="{{ old('email') }}"  required autocomplete="false" autofocus   maxlength="256"/>
	...
	<div class="inputGroup inputGroup2" style="margin-bottom: 21px;">
		<label for="password">Password</label>
		<input type="password" id="password" name="password" class="password" required autocomplete="new-password" autocomplete="false"  />
        @error('password')
            <span class="invalid-feedback" role="alert">
                <strong>{{ $message }}</strong>
            </span>
    ...
    <div class="inputGroup inputGroup2" style="margin: 0  0 22px 0;">
            <div class="form-check">
                <input class="form-check-input checkbox-custom" type="checkbox" name="remember" id="remember" {{ old('remember') ? 'checked' : '' }}>
                <label class="form-check-label checkbox-custom-label" for="remember">
                    {{ __('Remember Me') }}
                </label>
            </div>
    </div>
  ...
  <script src='https://cdnjs.cloudflare.com/ajax/libs/gsap/1.20.3/TweenMax.min.js'></script>
    ...
    <script  src="/assets/login2.js"></script>
